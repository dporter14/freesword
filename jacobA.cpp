...
void setPlayerOrientation()
{
    if (g.player.orientation[0] == -1) {
        if (g.player.orientation[1] == -1) {
            //face SW
            g.player.pointer[0] = g.player.pos[0] - (sqrt((PRADIUS*PRADIUS)/2));
            g.player.pointer[1] = g.player.pos[1] - (sqrt((PRADIUS*PRADIUS)/2));
        }
        else if (g.player.orientation[1] == 0) {
            //face West
            g.player.pointer[0] = g.player.pos[0] - PRADIUS;
            g.player.pointer[1] = g.player.pos[1];
        }
    ...
    else if (g.player.orientation[0] == 0) {
        if (g.player.orientation[1] == -1) {
            //face South
    ...
    else if (g.player.orientation[0] == 1) {
        if (g.player.orientation[1] == -1) {
            //face SE
            g.player.pointer[0] = g.player.pos[0] + (sqrt((PRADIUS*PRADIUS)/2));
            g.player.pointer[1] = g.player.pos[1] - (sqrt((PRADIUS*PRADIUS)/2));
...
void movePlayer()
{
    ...
    switch (g.player.dir) {
        case DIRECTION_W: 
            if (g.player.vel[1] < PLAYER_MAX_SPEED) {
                g.player.vel[1] += 10; 
            } else {
                g.player.vel[1] = PLAYER_MAX_SPEED;
            }
            break;
        case DIRECTION_A: 
            if (g.player.vel[0] > N_PLAYER_MAX_SPEED) {
                g.player.vel[0] += -10;
            } else {
                g.player.vel[0] = N_PLAYER_MAX_SPEED;
            }
            break;
        case DIRECTION_S: 
            if (g.player.vel[1] > N_PLAYER_MAX_SPEED) {
                g.player.vel[1] += -10; 
            } else { 
                g.player.vel[1] = N_PLAYER_MAX_SPEED;
            }
            break;
        case DIRECTION_D: 
            if (g.player.vel[0] < PLAYER_MAX_SPEED) {
                g.player.vel[0] += 10;
            } else {
                g.player.vel[0] = PLAYER_MAX_SPEED;
            }
            break;
    }
}
